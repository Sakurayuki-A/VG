...
public class VKDynamicTextVertexBuffer implements AutoCloseable {
    ...
    public VKDynamicTextVertexBuffer(VkDevice device, VKDevice vkDevice, int maxVertices) {
        this.device = device;
        this.vkDevice = vkDevice;
        this.bufferSize = maxVertices * 7L * 4L;
    ...
    public void updateVertices(float[] vertices, long dstOffset) {
        long dataSize = vertices.length * 4L;

        try (MemoryStack stack = MemoryStack.stackPush()) {
            LongBuffer pBuffer = stack.mallocLong(1);
            LongBuffer pMemory = stack.mallocLong(1);

            createBuffer(device, vkDevice, dataSize,
                    VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                    pBuffer, pMemory);
            long stagingBuffer = pBuffer.get(0);
            long stagingBufferMemory = pMemory.get(0);
            ...
            try {
                ...
                VKUtil.check(vkMapMemory(device, stagingBufferMemory, 0, dataSize, 0, pMappedMemory),
                        "Failed to map text staging buffer memory");

                MemoryUtil.memCopy(MemoryUtil.memAddress(vertexData), pMappedMemory.get(0), dataSize);
                vkUnmapMemory(device, stagingBufferMemory);

                copyBuffer(device, vkDevice, stagingBuffer, vertexBuffer, dataSize, dstOffset);
    ...
    private void createBuffer(VkDevice device, VKDevice vkDevice, long size, int usage,
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VKUtil.check(result, "Failed to create text buffer");
            ...
            VKUtil.check(result, "Failed to allocate text buffer memory");

            vkBindBufferMemory(device, pBuffer.get(0), pMemory.get(0), 0);
        }
    }

    private void copyBuffer(VkDevice device, VKDevice vkDevice, long srcBuffer, long dstBuffer, long size, long dstOffset) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VKUtil.check(result, "Failed to allocate text command buffer");
